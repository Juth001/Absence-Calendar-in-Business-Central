...
page 85001 "SVT Absence Overview"
{
        ...
        area(Content)
        {
            group(Filters)
            {
                ...
                field(EmployeeGroupFilter; EmployeeGroupFilter)
                {
                    ...
                    var
                        AbsenceSetup: Record "SVT Absence Registration Setup";
    ...
    var
        AbsRegSetup: Record "SVT Absence Registration Setup";
    ...
    begin
        ...
        if TempEmployeeAbsence.FindSet() then
            repeat
                if (ShowFutureAbsence = ShowFutureAbsence::All) or
                   ...
                   ((ShowFutureAbsence = ShowFutureAbsence::None) and (TempEmployeeAbsence."From Date" <= FromDate)) then
    ...
    local procedure EmployeeIsMemberOfDimension(inEmployeeCode: Code[20]; inEmployeeGroupDimensionCode: code[20]): Boolean
    var
    ...
    local procedure GetEmployeeGroupFilterFromRespCenter(inRespCenterCode: Code[10]; inEmployeeGroupDimension: Code[10]) ReturnCode: Code[20];
    var
        DefaultDim: Record "Default Dimension";
    begin
